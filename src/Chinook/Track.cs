...
namespace Chinook
{
    ...
    {
        /// <summary>
        /// The content type name of the <see cref="Chinook.Track"/>.
        /// </summary>
        public const string TrackContentTypeName = "Track";

        [ProtoContract]
        class TrackContent : IContent
        {
            [ProtoMember(1)]
            ...
            public Task<IHashableObject> ToHashableObjectAsync(string hash, IRepository repository, CancellationToken cancellationToken = default)
            {
                var track = new Track(repository, hash)
                {
                    ...
                    MediaType = new LazyHashableObject(repository, this.MediaTypeFullHash, Chinook.MediaType.MediaTypeContentTypeName),
                    Genre = new LazyHashableObject(repository, this.GenreFullHash, Chinook.Genre.GenreContentTypeName)
                };
        ...
        public Track(IRepository repository, string hash = null)
            : base(repository, TrackContentTypeName, hash)
        {
        }

        public static Type GetContentType() => typeof(TrackContent);
    ...
    public class LazyTrack : LazyHashableObject
    {
        public LazyTrack(IRepository repository, string fullHash)
            : base(repository, fullHash, Track.TrackContentTypeName)
        {
        }
