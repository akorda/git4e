    ...
    public class CommitsComparer : ICommitsComparer
    {
        ...
        private static async Task CompareObjectsAsync(ICommit commit, ICommitsComparerVisitor visitor, IHashableObject obj1, IHashableObject obj2, CancellationToken cancellationToken)
        {
            ...
            foreach (var property in contentCollectionProperties)
            {
                ...
                {
                    await visitor.OnCollectionsDifferAsync(commit, collection1.GetItems(), collection2.GetItems(), cancellationToken);

                    //find albums in lib1 but not in lib2 and updated albums
                    foreach (var item1 in collection1.GetItems())
                    {
                        var item2 = collection2.GetItems().FirstOrDefault(a => a.UniqueId == item1.UniqueId);
                        if (item2 == null)
                        {
                    ...
                    foreach (var item2 in collection2.GetItems())
                    {
                        var item1 = collection1.GetItems().FirstOrDefault(a => a.UniqueId == item2.UniqueId);
                        if (item1 == null)
                        {
